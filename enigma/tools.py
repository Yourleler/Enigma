...
BASE_TOOL_SPECS = {
    ...
    "web_search": {
        "schema": {"query": "str", "max_results": "int=5", "allowed_domains": "str=''", "blocked_domains": "str=''"},
        "risky": False,
        "description": "Search the web and return sourced result snippets.",
    },
    "run_shell": {
...
def validate_tool(agent, name, args):
    ...
    if name == "web_search":
        ...
        if max_results < 1 or max_results > 10:
            raise ValueError("max_results must be in [1, 10]")
        return
...
def tool_web_search(agent, args):
    ...
    if max_results < 1 or max_results > 10:
        raise ValueError("max_results must be in [1, 10]")
    allowed_domains = web_search.parse_domain_list(args.get("allowed_domains", ""))
    blocked_domains = web_search.parse_domain_list(args.get("blocked_domains", ""))
    try:
        results = web_search.default_search_client.search(
            query,
            max_results,
    ...
    for index, result in enumerate(results[:max_results], start=1):
        ...
        snippet = str(result.snippet).strip() or "(no snippet)"
        lines.extend(
            [
                f"{index}. {title}",
                f"   url: {url}",
...
def tool_run_shell(agent, args):
    ...
    if timeout < 1 or timeout > 120:
        raise ValueError("timeout must be in [1, 120]")
    run_kwargs = {}
    shell_executable = getattr(agent, "shell_executable", lambda: None)()
    if shell_executable:
