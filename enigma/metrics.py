...
from .runtime import Enigma, SessionStore
...
DEFAULT_CORE_REPORT_PATH = Path("docs/metrics/enigma-benchmark-core-report.md")
...
def build_stress_agent_metrics():
    with tempfile.TemporaryDirectory(prefix="enigma-metrics-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        workspace = WorkspaceContext.build(workspace_root)
        store = SessionStore(workspace_root / ".enigma" / "sessions")
        agent = Enigma(
...
def _run_memory_variant(mode):
    with tempfile.TemporaryDirectory(prefix="enigma-memory-experiment-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _run_memory_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="enigma-memory-large-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="enigma-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".enigma" / "sessions")
...
def _scenario_repeated_call(workspace_root):
    ...
    for _ in range(2):
        ...
        agent.append_session_history({"role": "tool", "name": "read_file", "args": args, "content": result, "created_at": "2026-04-09T00:00:00+00:00"})
...
def run_security_experiment_suite(repetitions=3):
    ...
    for scenario_id, runner in SECURITY_SCENARIOS:
        for _ in range(repetitions):
            with tempfile.TemporaryDirectory(prefix="enigma-security-exp-") as temp_dir:
                workspace_root = Path(temp_dir)
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="enigma-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="enigma-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _run_real_repeated_call_scenario(provider):
    with tempfile.TemporaryDirectory(prefix="enigma-real-security-repeat-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with tempfile.TemporaryDirectory(prefix="enigma-real-security-") as temp_dir:
                workspace_root = Path(temp_dir)
...
def render_large_scale_experiment_report(metrics):
    ...
    lines = [
        "# Enigma Large-Scale Experiment Report",
        "",
        "## Executive Summary",
        (
...
def _run_recovery_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="enigma-recovery-ablation-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def write_benchmark_core_report(
    ...
    lines = [
        "# Enigma Benchmark Core Report",
        "",
        "这轮 benchmark 只收缩到 Harness regression、context ablation、工作记忆 ablation 和 recovery ablation 四层，不把 provider、run aggregation 或语义记忆的别的结论揉进来。",
        "",
        "## Harness Regression",
